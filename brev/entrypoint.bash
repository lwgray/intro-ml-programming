...
if [ "$(id -u)" = "0" ]; then
    TARGET_USER="${IML_USER:-student}"
    TARGET_UID="${IML_UID:-1000}"
    TARGET_GID="${IML_GID:-1000}"
    ...
    export IML_TARGET_USER="${TARGET_USER}"
    export IML_TARGET_HOME=$(getent passwd "${TARGET_USER}" | cut -d: -f6 || true)
    ...
    if [ ! -e "${HOME}/.jupyter/jupyter_server_config.py" ]; then
        gosu "${TARGET_USER}" ln -sf /intro-ml-programming/brev/jupyter-server-config.py "${HOME}/.jupyter/jupyter_server_config.py"
    fi

    # Link IPython startup scripts if not already present
    if [ ! -e "${HOME}/.ipython/profile_default/startup/00-add-cwd-to-path.py" ]; then
        gosu "${TARGET_USER}" ln -sf /intro-ml-programming/brev/ipython-startup-add-cwd-to-path.py "${HOME}/.ipython/profile_default/startup/00-add-cwd-to-path.py"
    fi
    # Setup Git safe directory (run as target user)
    gosu "${TARGET_USER}" git config --global --add safe.directory "/intro-ml-programming" 2>/dev/null || true

    # Ensure logs directory exists and is writable by the target user.
    mkdir -p /intro-ml-programming/logs
    chown "${TARGET_USER}" /intro-ml-programming/logs
fi

# Dispatch to service-specific entrypoint
SERVICE_ENTRYPOINT="/intro-ml-programming/brev/entrypoint-${SERVICE}.bash"
